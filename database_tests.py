...
class BaseTestCase(TestCase):
    ...
    def test_set_points_for_round(self):
        ...
        self.assertEqual(
            Game_with_Songs.query.filter_by(game=game_id).filter_by(round_numb=0).first().points, 1)
        self.assertEqual(
            Game_with_Songs.query.filter_by(game=game_id).filter_by(round_numb=1).first().points, 3)
