...
# Set up the Database with on table for the player, one that link the player to a game and one
# that links a game to songs and scores.
...
def calculate_points(song_number, title_input, artist_input):
    ...
    if answer_is_correct(title_input, song_number, 'title') and answer_is_correct(artist_input,
            song_number, 'artist'):
        return 3
    elif answer_is_correct(title_input, song_number, 'title') or answer_is_correct(artist_input,
            song_number, 'artist'):
        return 1
    else:
        return 0
...
def set_points_for_round(player_id, current_round, points):
    ...
    """
    game_id = get_game_id(player_id)
    song_entry = Game_with_Songs.query.filter_by(game=game_id).filter_by(
        round_numb=current_round).first()
...
def get_result_data(game_id):
    ...
    for song_entry in playlist:
        song = {
            ...
            'points': Game_with_Songs.query.filter_by(game=game_id).filter_by(
                song=song_entry.song).first().points
        }
...
def index():
    if request.method == "POST":
        ...
        if username:
            ...
            playlist = get_playlist(game_id)
            return redirect(
                url_for('play_song', player_id=int(player_id), song_number=int(playlist[0].song)))
...
def play_song(player_id, song_number):
    ...
    if request.method == "POST":
        ...
        if current_round < 9:
            points = calculate_points(song_number=song_number, title_input=title_guess,
                artist_input=artist_guess)
            set_points_for_round(player_id=player_id, current_round=current_round, points=points)
            increase_round_counter(player_id)
            game_id = get_game_id(player_id)
            playlist = get_playlist(game_id)
            return redirect(url_for('play_song', player_id=int(player_id),
                song_number=int(playlist[current_round + 1].song)))
    ...
    return render_template('playing.html', song=songs_array[song_number]["preview_url"],
        current_round=current_round, total_points=total_points)
...
def result(player_id):
    ...
    total_points = get_total_points(game_id)
    return render_template('result.html', player_id=player_id, result_data=result_data,
